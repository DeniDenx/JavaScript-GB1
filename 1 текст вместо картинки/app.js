...
function handleClick(clickedButtonEvent) {
    ...
    let cardText = card.button.innerHTML;


    if (cardText == 'Подробнее') { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (cardText == 'Отмена') { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    let anyText = 'Любой текст';
    // 6.1 сохраняем произвольный текст в переменную
    card.productName.insertAdjacentHTML('afterend', `<div class ='desc'>${anyText}</div>`);
